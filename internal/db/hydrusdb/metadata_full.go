...
func (b *Bundle) fullMetadataRowsWithConn(
	ctx context.Context,
	conn *sql.Conn,
	orderedHashes []string,
	hashesToFileIDs map[string]int64,
	...
	knownFileIDs := dedupeInt64s(mapValues(hashesToFileIDs))
	basicRecords, err := b.lookupBasicRecordsConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}

	pixelHashes, err := b.lookupPixelHashesConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}

	transparencyHashIDs, err := b.lookupHashIDSetConn(ctx, conn, knownFileIDs, "main.has_transparency", "has_transparency")
	if err != nil {
		return nil, err
	}

	exifHashIDs, err := b.lookupHashIDSetConn(ctx, conn, knownFileIDs, "main.has_exif", "has_exif")
	if err != nil {
		return nil, err
	}

	humanReadableHashIDs, err := b.lookupHashIDSetConn(ctx, conn, knownFileIDs, "main.has_human_readable_embedded_metadata", "has_human_readable_embedded_metadata")
	if err != nil {
		return nil, err
	}

	iccProfileHashIDs, err := b.lookupHashIDSetConn(ctx, conn, knownFileIDs, "main.has_icc_profile", "has_icc_profile")
	if err != nil {
		return nil, err
	}

	inboxHashIDs, err := b.lookupHashIDSetConn(ctx, conn, knownFileIDs, "main.file_inbox", "file_inbox")
	if err != nil {
		return nil, err
	}

	archivedTimestamps, err := b.lookupInt64ByHashIDConn(ctx, conn, knownFileIDs, "main.archive_timestamps", "archived_timestamp_ms", "archive_timestamps")
	if err != nil {
		return nil, err
	}

	fileModifiedTimestamps, err := b.lookupInt64ByHashIDConn(ctx, conn, knownFileIDs, "main.file_modified_timestamps", "file_modified_timestamp_ms", "file_modified_timestamps")
	if err != nil {
		return nil, err
	}

	domainModifiedTimestamps, err := b.lookupDomainModifiedTimestampsConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}

	knownURLs, err := b.lookupKnownURLsConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}
	...
	currentFileServices, deletedFileServices, err := b.lookupFileServiceMembershipsConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}

	ipfsMultihashes, err := b.lookupIPFSMultihashesConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}
	...
	notesByHashID := map[int64]map[string]string{}
	if includeNotes {
		notesByHashID, err = b.lookupFileNotesConn(ctx, conn, knownFileIDs)
		if err != nil {
			return nil, err
		}
	}

	ratingsByHashID, err := b.lookupFileRatingsConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}

	viewingStatsByHashID, err := b.lookupFileViewingStatisticsConn(ctx, conn, knownFileIDs)
	if err != nil {
		return nil, err
	}

	tagsByHashID, err := b.lookupFileTagsConn(ctx, conn, knownFileIDs, currentFileServices)
	if err != nil {
		return nil, err
	}
...
func (b *Bundle) lookupPixelHashesConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
) (map[int64]string, error) {
...
func (b *Bundle) lookupHashIDSetConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
	tableName string,
...
func (b *Bundle) lookupInt64ByHashIDConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
	tableName string,
...
func (b *Bundle) lookupDomainModifiedTimestampsConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
) (map[int64][]domainModifiedTimestamp, error) {
...
func (b *Bundle) lookupKnownURLsConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
) (map[int64][]string, error) {
...
func (b *Bundle) lookupFileServiceMembershipsConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
) (
	...
	servicesByID, err := b.cachedAllServiceDefinitions(ctx, conn)
	if err != nil {
		return nil, nil, err
	}

	tableNames, err := b.lookupMainTableNamesConn(ctx, conn)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, service := range servicesByID {
		currentTableName := fmt.Sprintf("current_files_%d", service.id)
		if _, ok := tableNames[currentTableName]; ok {
			memberships, err := b.lookupCurrentFileServiceMembershipsConn(ctx, conn, fileIDs, currentTableName, service)
			if err != nil {
				return nil, nil, err
			}
		...
		deletedTableName := fmt.Sprintf("deleted_files_%d", service.id)
		if _, ok := tableNames[deletedTableName]; ok {
			memberships, err := b.lookupDeletedFileServiceMembershipsConn(ctx, conn, fileIDs, deletedTableName, service)
			if err != nil {
				return nil, nil, err
			}
...
func (b *Bundle) lookupCurrentFileServiceMembershipsConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
	tableName string,
...
func (b *Bundle) lookupDeletedFileServiceMembershipsConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
	tableName string,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&hashID, &deletedTimestampMS, &originalImportedTimestamp); err != nil {
			return nil, fmt.Errorf("scan deleted file service %s row: %w", tableName, err)
		}
...
func (b *Bundle) lookupIPFSMultihashesConn(
	ctx context.Context,
	conn *sql.Conn,
	fileIDs []int64,
...
func (b *Bundle) lookupMainTableNamesConn(ctx context.Context, conn *sql.Conn) (map[string]struct{}, error) {
	return b.lookupSchemaTableNamesConn(ctx, conn, "main")
}

func (b *Bundle) lookupSchemaTableNamesConn(
	ctx context.Context,
	conn *sql.Conn,
	schemaName string,
...
func (b *Bundle) cachedAllServiceDefinitions(ctx context.Context, conn *sql.Conn) ([]serviceDefinition, error) {
	var retErr error
	b.serviceDefsOnce.Do(func() {
		defs, err := lookupAllServiceDefinitionsQuerier(ctx, conn)
		b.serviceDefsVal = defs
		b.serviceDefsErr = err
	})
	if retErr != nil {
		return nil, retErr
	}
	if b.serviceDefsErr != nil {
		return nil, b.serviceDefsErr
	}
	return b.serviceDefsVal, nil
}

func (b *Bundle) cachedMasterTagSchemaMode(ctx context.Context, conn *sql.Conn) (masterTagSchemaMode, error) {
	b.tagSchemaModeOnce.Do(func() {
		tableNames, err := b.lookupSchemaTableNamesConn(ctx, conn, "external_master")
		if err != nil {
			b.tagSchemaModeErr = err
			return
		}

		tagColumns, err := lookupTagColumns(ctx, conn, `PRAGMA external_master.table_info(tags)`, tableNames)
		if err != nil {
			b.tagSchemaModeErr = err
			return
		}

		mode, err := masterTagSchemaModeFromTableNames(tableNames, tagColumns)
		b.tagSchemaModeVal = mode
		b.tagSchemaModeErr = err
	})
	if b.tagSchemaModeErr != nil {
		return masterTagSchemaEmpty, b.tagSchemaModeErr
	}
	return b.tagSchemaModeVal, nil
}

func (b *Bundle) lookupAllServiceDefinitions(ctx context.Context) ([]serviceDefinition, error) {
