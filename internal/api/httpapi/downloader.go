...
func (s *Server) handlePostDownloaderGallery(w http.ResponseWriter, r *http.Request) {
	_, statusCode, err := s.access.Authorize(r, PermissionImportAndEditURLs)
	if err != nil {
		writeError(w, statusCode, err.Error())
		return
	...
	request, err := parseDownloaderGalleryRequest(r)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	if err := s.downloaderStore.QueueGallery(r.Context(), request); err != nil {
		if writeDownloaderStoreError(w, err) {
			return
		}
	}
...
func parseDownloaderGalleryRequest(r *http.Request) (coredownloader.GalleryRequest, error) {
	decoder := json.NewDecoder(r.Body)
	decoder.DisallowUnknownFields()

	var request coredownloader.GalleryRequest
	if err := decoder.Decode(&request); err != nil {
		return coredownloader.GalleryRequest{}, err
	}
	if err := decoder.Decode(&struct{}{}); err != io.EOF {
		return coredownloader.GalleryRequest{}, errors.New("request body must contain a single JSON object")
	}
